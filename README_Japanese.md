...
> 上書き前の自動バックアップ (後述の注意事項参照) は、レンダリング後の
> ヘッダー行 (`# YYYY/MM/DD ...` で始まる行) を各 Q&A ブロックの
> 識別子として使います。旧ファイルにあった識別子が新しい内容から
> *消える* とき (=本当に内容が失われるとき) だけバックアップします。
> カスタムテンプレートが `# %DateTime%` で始まる行を出力しない場合、
> どのブロックにも識別子が無くなり検知が働かなくなります —
> **セッションが本当に消える場面でもバックアップが二度と作られません**
> (例: `.jsonl` が削除された場合)。同梱テンプレート 6 種はすべて
> この形式を守っています。カスタマイズするときは、ヘッダー行より
> *下* は自由に変えて構いませんが、ブロック先頭の `# %DateTime%`
> だけは残してください。
...
- **上書き前の Markdown バックアップ。** 既存の出力 `.md` の書き換えが
  *破壊的* な場合 — 旧ファイルにあった Q&A ブロックのうち少なくとも
  1 つが新しい内容から消えている (`# YYYY/MM/DD ...` のヘッダー行が
  見つからなくなる。例: セッションの `.jsonl` が削除された、一部の
  セッションが見えない別 PC で実行した、など) — 上書きの直前に
  既存ファイルを
