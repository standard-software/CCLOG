...
> ### 📦 cclog は `ccxlog` へ移行します — 乗り換えのお願い
>
> **[`@standard-software/ccxlog`](https://www.npmjs.com/package/@standard-software/ccxlog)** が cclog の後継です。**cclog は Claude Code のログ専用**ですが、**ccxlog は Claude Code と Codex CLI の両方**のログを 1 つのコマンドで出力でき、1 つのタイムラインに統合することもできます。
>
> - `ccxlog -cc`(Claude Code のみ)は `cclog` とほぼ同等です。
> - `ccxlog -cx` は Codex 用で、デフォルトは両方を統合します。
>
> 今後の開発は **ccxlog** で継続するので、cclog からの乗り換えをお願いします:
>
> ```bash
...
配下に書き出している JSONL を読み込み、プロジェクト配下に `cclog.md`
...
`cclog.md` やセッション別ファイルは生成・更新**しません**
...
| `includeSubdirectories`    | `true`(デフォルト)のとき、cclog を実行したプロジェクトの *サブディレクトリ* にあたる cwd を持つプロジェクトのログも収集します(例: `~/work/app` で実行すると `~/work/app/frontend` も集約)。候補はログフォルダ名の前方一致で見つけたうえで各セッションの実 cwd で検証するので、`~/work/app-backup` のような同一接頭辞の兄弟プロジェクトは混入しません。`false` にすると、完全一致したプロジェクトパス(と `extraCwds` / `extraLogDirs`)だけを対象とする従来動作に戻ります。 |
| `outputAllFileName`        | 集約出力ファイル名。デフォルト `cclog.md`。ファイル内の見出しはこのファイル名 (拡張子除く) から生成されます (例: `mylog.md` にすると見出しも `# mylog` になります)。 |
...
1.6.0 より前の出力ファイル名 (`CCLOG_ALL.md` と `CCLOG_<sessionId>.md`) の
ままにしたい場合は、次のように設定してください:
...
| `%Model%`         | 回答を生成したモデル (`claude-opus-4-8`)。合成エントリはスキップ |
| `%Version%`       | そのペアを処理した Claude Code のバージョン (`2.1.205`)     |
| `%GitBranch%`     | 質問時点の git ブランチ                                     |
| `%Cwd%`           | 質問時点の作業ディレクトリ                                  |
| `%Tokens%`        | ペア内の assistant ターンを合算したトークン使用量 (`in 6, out 33, cache read 21,758, cache write 8,730`) |
...
> **⚠️ 先頭行は必ず `# %DateTime%` で始めてください。**
> 上書き前の自動バックアップ (後述の注意事項参照) は、ヘッダー行の
> `# YYYY/MM/DD Day HH:MM:SS` プレフィックス**だけ**を各 Q&A ブロックの
> 識別子として使います — 同じ行のタイムスタンプより後ろは無視されます。
> 質問のタイムスタンプはペアごとに固定で変わらないため、テンプレート
> 変更 (ヘッダー行の後半を変えても) やセッション名の変更でバックアップが
> 発生することはありません。旧ファイルにあったタイムスタンプが新しい
> 内容から *消える* とき (=ペアが本当に失われるとき) だけバックアップ
> します。カスタムテンプレートが `# %DateTime%` で始まる行を出力しない
> 場合、どのブロックにも識別子が無くなり検知が働かなくなります —
> **セッションが本当に消える場面でもバックアップが二度と作られません**
> (例: `.jsonl` が削除された場合)。同梱テンプレート 6 種はすべて
> この形式を守っています。カスタマイズするときは、`%DateTime%` より
> 後ろ (同じ行でも下の行でも) は自由に変えて構いませんが、ブロックは
> `# %DateTime%` で始めてください。

## 出力フォーマット

`cclog.md` は Q&A ブロックがフラットに時系列で並びます。各ブロックは
...
見出し行のコロンに挟まれた部分が `%SessionName%` (セッション名) です。
Claude Code でカスタムタイトルを付けていればそれ、無ければ自動生成
タイトル、どちらも無ければ空 (`Session::<uuid>`) になります。
...
- 出力は毎回完全に再生成されます。`~/.claude/projects/...` 配下の
  セッションログを削除すると、次回 `cclog` 実行時に `cclog.md` から
  該当ペアも消えます。
- **上書き前の Markdown バックアップ。** 既存の出力 `.md` の書き換えが
  *破壊的* な場合 — 旧ファイルにあった Q&A ブロックのうち少なくとも
  1 つが新しい内容から消えている (`# YYYY/MM/DD Day HH:MM:SS` の
  タイムスタンプがどのヘッダー行にも見つからなくなる。例: セッションの
  `.jsonl` が削除された、一部のセッションが見えない別 PC で実行した、
  など) — 上書きの直前に
  既存ファイルを
  `CCLOG/backup_CCLOG_md/<yyyy-mm-dd_hh-mm-ss>_<hostname>/` へコピーします。
  これにより以前の版が失われません。バックアップフォルダは蓄積され、
  自動削除されることはありません。通常の追記・変更なしの実行・初回作成、
  および全ブロックが保たれる書き換え (例: ヘッダー行より下だけの
  テンプレート変更) ではバックアップを取らないので、このフォルダは
  実際に内容が消えたときだけ作成されます。この検知はテンプレートの
  ヘッダー行に依存します — 「テンプレートをカスタマイズする」の
  警告を参照してください。
  許容している既知のエッジケース: 同一秒に打刻された 2 ペアは識別子を
  共有します。2 つが共存している分には無害です — 両方とも出力され、
  誤バックアップも発生しません。理論上の検知漏れは「同一秒の双子の
  片方だけが消え、もう片方が生き残った」場合のみで、生き残りが
  そのタイムスタンプを保持しているため損失が検知されずバックアップが
  取られません。秒単位の衝突でかつ片方だけが消えるという状況は
  まず起きないため、予測可能なバックアップ動作と引き換えに許容して
  います。
