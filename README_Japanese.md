...
オプション:
  ...
  --init-template        同梱テンプレートを <out>/templates/ にコピーし、
                         ...
                         テンプレートを編集したいとき用)。
  --backup-jsonl         検出した元の .jsonl ログを、出力前に
                         <out>/backup_jsonl/<yyyy-mm-dd_hh-mm-ss>/ へ
                         コピーします (生のログを手元に退避。PC 入れ替え前
                         などに有効。元ログの保存場所はマシン依存の
                         プロジェクトパスから決まるため)。
...
検出したすべての `.jsonl` を
`CCLOG/backup_jsonl/<yyyy-mm-dd_hh-mm-ss>/` (実行ごとに新しいタイムスタンプ
フォルダ) にコピーしてから、通常どおり出力します。各バックアップは
セッション元の `<uuid>.jsonl` というファイル名を保つので、後から再利用
できます。`--dry-run` と併用するとコピーせずにコピー先だけを確認でき、
